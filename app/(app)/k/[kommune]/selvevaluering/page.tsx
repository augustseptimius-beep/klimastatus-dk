...
import { genererSelvevalueringFormAction, saveKriterieBesvarelse, godkendKriterie } from './actions';
...
export default async function SelvevalueringPage({ params }: Props) {
  ...
  const liveDokRefsAll = await Promise.all(
    kriterier.map(k => getDokumentationshenvisninger(kommune.id, k.kriterieNr))
  );
  ...
  const boundGenerer = genererSelvevalueringFormAction.bind(null, slug);
  const boundSave = saveKriterieBesvarelse.bind(null, slug);
  const boundGodkend = godkendKriterie.bind(null, slug);
    ...
    <div style={{ padding: '32px 40px', maxWidth: 960 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <Link href={`/k/${slug}/selvevaluering/preview`} style={{
            padding: '8px 16px', borderRadius: 4,

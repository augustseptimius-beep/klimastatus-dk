...
vi.mock('@/db', () => ({
  db: {
    query: {
      tovholderRapport: {
        findMany: vi.fn().mockResolvedValue([{ id: 'r1', tiltagId: 't1', tovholderId: 'th1', dato: '2026-05-11', createdAt: new Date(), barrierer: null, statusImplementering: null, statusBeskrivelse: null, naesteSkrid: null, effektRealiseret: null, forespoergselId: null }]),
...
describe('upsertRapport', () => {
  ...
  it('updates existing rapport when findFirst returns one', async () => {
    const { db } = await import('@/db');
    vi.mocked(db.query.tovholderRapport.findFirst).mockResolvedValueOnce({ id: 'r1', tiltagId: 't1', tovholderId: 'th1', dato: '2026-05-11', createdAt: new Date(), barrierer: null, statusImplementering: null, statusBeskrivelse: null, naesteSkrid: null, effektRealiseret: null, forespoergselId: null });
